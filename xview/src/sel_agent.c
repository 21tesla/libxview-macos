    ...
    Seln_agent_info *agent;
{
	...
	*context->response_pointer++ = (char *) stat_buf.st_size;
	...
	if (size > max_length) {
	    count = darwinread(fd, (char *) context->response_pointer, max_length);
	    if (count != max_length) {
	    ...
	    count = darwinread(fd, (char *) context->response_pointer, size);
    terminate_buffer:
...
#endif
#ifndef SVR4
	result = darwinselect(max_fds, &select_ibits,
			(fd_set *) NULL, (fd_set *) NULL, timeout);
#else /* SVR4 */
	result = darwinselect(display->fd + 1, &select_ibits, NULL, NULL, timeout);
#endif /* SVR4 */
