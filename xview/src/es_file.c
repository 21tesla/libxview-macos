...
extern int      errno;
/* removed , sys_nerr; from above    Logan  */
...
/* ARGSUSED */
caddr_t
/* removed static from above   Logan  */
    ...
    char           *caller;
{
    ...
    if ((multibyte) || (private->options & ES_OPT_BACKUPFILE))
        pos *= sizeof(CHAR);
#endif /* OW_I18N */
    if (lseek(private->fd, (off_t) pos, SEEK_SET) == -1) {
    ...
    register Es_index first, last_plus_one;
{
	...
	if (es_file_seek(private, first, "es_file_fill_buf")) {
        ...
        /* Backup file or multibyte locale, read in without converting */
            read_in = darwinread(private->fd, buf->chars,
            ...
            read_in = darwinread(private->fd, temp_buf_ptr, last_plus_one - first);
            if (read_in > 0) {
...
#else /* OW_I18N */
	read_in = darwinread(private->fd, buf->chars, last_plus_one - first);
#endif /* OW_I18N */
	if (read_in == -1 ||
 ...
 */
{
	...
	if (es_file_seek(private, buf->start, "es_file_flush_write_buf")) {
        ...
        written = write(private->fd, buf->chars, (size_t) num_of_bytes);       
    } else {
        /* 
	...
	written = write(private->fd, temp_buf_ptr, (size_t) num_of_bytes);
