...
#else
    if ((fdflags = darwinfcntl(fd, F_GETFL, 0)) == -1)
#endif
	return (-1);
...
#else
    if (darwinfcntl(fd, F_SETFL, fdflags) == -1)
#endif
	return (-1);
    return (0);
}
    ...
    int             ttyslotuse, ttyfd;
{
	...
	(void) lseek(f, (off_t) (ttyslotuse * sizeof(utmp)), 0);
	(void) write(f, (char *) &utmp, (size_t) sizeof(utmp));
	(void) close(f);
    } else {
	(void) fprintf(stderr, 
