...
/* extern CHAR    *STRCAT();  logan */
/* extern CHAR    *STRNCAT(); logan */
#ifdef SVR4
extern char    *getcwd();
#else
extern char    *getwd();
#endif /* SVR4 */
extern int      errno;
/* took this out from above , sys_nerr;   Logan */

/* extern char    *sys_errlist[]; */
/* commented out this extern   Logan */
...
#endif
{
    ...
    for (;;) {
	count = darwinread(fd, buf, sizeof(buf) - 1);
	if (count == 0)
	    break;
	if (count < 0) {
	    return (ES_UNKNOWN_ERROR);
...
#else
	    new_pos = lseek(fd, (off_t) (temp_count - count), L_INCR); 
#endif
	    if (new_pos == old_pos) {
...
#else
		old_pos = lseek(fd, (off_t) 1, L_INCR);
#endif
		skipped = 1;
	    } else
		old_pos = new_pos;
