...
#define	NR_LAST_PLUS_ONE	1
	...
	tv.tv_sec = 15;
	tv.tv_usec = 0;
	nfds = darwinselect(max_fds, &readfds, &writefds, &exceptfds, &tv);
	switch (nfds) {
	  case -1:
	    if (errno == EINTR)
	...
	if((filter_output >= 0) && (FD_ISSET(filter_output, &readfds))) {
	    ...
	    bytes_read = darwinread(filter_output,
...
#else /* OW_I18N */
	    bytes_read = darwinread(filter_output, (char *) buffer,
			      sizeof(buffer_and_slop) - SLOP_SIZE);
	    switch (bytes_read) {
#endif /* OW_I18N */
	      case 0:
...
#ifdef OW_I18N
	{
		...
		written[0] = write(filter_input, mb_buf, (size_t) to_write_mb);
    Write_check:
		if (written[0] == -1) {
		    if (errno == EMSGSIZE) {
...
#define	INSERT_LINE_WRITTEN	2
	...
	nfds = darwinselect(max_fds, &readfds, &writefds, &exceptfds, tv_to_use);
	switch (nfds) {
	  case -1:
	    if (errno == EINTR)
		continue;	/* Probably itimer expired */
	...
	if (FD_ISSET(filter_input, &writefds)) {
		...
		if ((write(filter_input, (char *) &filter_attribute,
			   (size_t) sizeof(filter_attribute)) == -1) ||
		    (write(filter_input, (char *) event,
			   (size_t) sizeof(*event)) == -1))
		    goto Write_Failed;
...
#else
    if (darwinfcntl(to_filter[OUTPUT], F_SETFL, FNDELAY) == -1)
#endif
	return (FAIL);
...
#else
    if (darwinfcntl(from_filter[INPUT], F_SETFL, FNDELAY) == -1)
#endif
	return (FAIL);
