...
#else
	    new_pos = lseek(fold, (off_t) ( temp - n), L_INCR);
#endif /* SVR4 */
	    if (new_pos == old_pos) {
...
#else
	        old_pos = lseek(fold, (off_t) 1, L_INCR);   
#endif /* SVR4 */
		*skipped = 1;
	    } else
	        old_pos = new_pos;
	}
	if ((write(fnew, (char *)buf_ws, (size_t)(len_in_wc * sizeof(CHAR)))
